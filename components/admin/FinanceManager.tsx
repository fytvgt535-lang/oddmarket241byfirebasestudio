...
const FinanceManager: React.FC<FinanceManagerProps> = ({ markets, expenses, onAddExpense, onDeleteExpense, loading = false, currentLanguage = 'fr' }) => {
  // DATE FILTERS
    ...
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card className="p-5 border-l-4 border-l-green-500">
                <p className="text-xs font-bold text-gray-400 uppercase mb-1">{t(currentLanguage, 'fin_revenue_period')}</p>
            ...
            <Card className="p-5 border-l-4 border-l-red-500">
                <p className="text-xs font-bold text-gray-400 uppercase mb-1">{t(currentLanguage, 'fin_total_expenses')}</p>
            ...
            <Card className="p-5 border-l-4 border-l-blue-500">
                <p className="text-xs font-bold text-gray-400 uppercase mb-1">{t(currentLanguage, 'fin_net_balance')}</p>
        ...
        <Card className="p-4 flex flex-col md:flex-row gap-4 items-end justify-between">
            <div className="flex gap-4 items-end w-full md:w-auto">
                <Input label={t(currentLanguage, 'fin_from')} type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-40"/>
                <Input label={t(currentLanguage, 'fin_to')} type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="w-40"/>
            ...
            <Button variant="outline" leftIcon={Download}>{t(currentLanguage, 'fin_export')}</Button>
        </Card>

        {/* Charts */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card className="p-6">
                <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2"><TrendingUp className="w-5 h-5 text-blue-600"/> {t(currentLanguage, 'fin_evolution')}</h3>
                <div className="h-64">
            ...
            <Card className="p-6 flex flex-col">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="font-bold text-gray-900 flex items-center gap-2"><Wallet className="w-5 h-5 text-red-600"/> {t(currentLanguage, 'fin_expenses')}</h3>
                    <Button size="sm" variant="danger" leftIcon={Plus} onClick={() => setIsExpenseModalOpen(true)}>{t(currentLanguage, 'add')}</Button>
                </div>
        ...
        <Card className="overflow-hidden">
            <table className="w-full text-sm text-left text-gray-700">
                <thead className="bg-gray-50 text-gray-500 font-bold border-b">
                    <tr><th className="p-4">{t(currentLanguage, 'date')}</th><th className="p-4">Réf</th><th className="p-4">Type</th><th className="p-4">Étal</th><th className="p-4 text-right">{t(currentLanguage, 'fin_amount')}</th></tr>
                </thead>
            ...
            <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
                <div className="bg-white rounded-2xl w-full max-w-md p-8 shadow-2xl">
                    ...
                    <form onSubmit={handleAddExpense} className="space-y-5">
                        <Select label={t(currentLanguage, 'user_market')} value={expenseForm.marketId} onChange={e => setExpenseForm({...expenseForm, marketId: e.target.value})}>
                            <option value="">Choisir Marché...</option>
                            {markets.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
                        </Select>
                        <Select label={t(currentLanguage, 'fin_category')} value={expenseForm.category} onChange={e => setExpenseForm({...expenseForm, category: e.target.value as any})}>
                        ...
                        <Input label={t(currentLanguage, 'fin_amount')} type="number" required placeholder="0" value={expenseForm.amount} onChange={e => setExpenseForm({...expenseForm, amount: e.target.value})}/>
                        ...
                        <Input label={t(currentLanguage, 'date')} type="date" value={expenseForm.date} onChange={e => setExpenseForm({...expenseForm, date: e.target.value})}/>
                        <div className="flex gap-3 pt-2">
                            <Button type="button" variant="ghost" onClick={() => setIsExpenseModalOpen(false)} className="flex-1">{t(currentLanguage, 'cancel')}</Button>
                            <Button type="submit" variant="danger" className="flex-1">{t(currentLanguage, 'add')}</Button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
                <div className="bg-white rounded-2xl w-full max-w-sm p-6 text-center">
                    ...
                    <div className="flex gap-3">
                        <Button variant="ghost" onClick={() => setDeleteExpenseId(null)} className="flex-1">{t(currentLanguage, 'cancel')}</Button>
                        <Button variant="danger" onClick={handleDeleteExpense} className="flex-1">{t(currentLanguage, 'delete')}</Button>
                    </div>
                </div>
            </div>
        )}
