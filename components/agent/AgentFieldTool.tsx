...
import { 
  ...
  Loader2, Sparkles, RefreshCw, AlertTriangle, Info, History, CloudUpload, CloudOff
} from 'lucide-react';
...
const AgentFieldTool: React.FC<AgentFieldToolProps> = ({
  ...
  useEffect(() => {
      const checkQueue = () => setPendingSync(getOfflineQueueSize());
      checkQueue();
  ...
  const handleManualSync = async () => {
      if (!navigator.onLine) return toast.error("Internet requis pour synchroniser.");
      setIsSyncing(true);
      const count = await syncOfflineQueue();
      setIsSyncing(false);
      if (count > 0) toast.success(`${count} opérations synchronisées !`);
      setPendingSync(0);
  };
  ...
  return (
    <div className={`min-h-screen -m-4 p-4 transition-colors duration-300 ${solarisClass}`}>
        ...
        {/* TOP SAFETY & SYNC BAR */}
        <div className="flex gap-2 mb-6 sticky top-0 z-50 bg-inherit/90 backdrop-blur-md py-2 px-1">
            <button 
                onClick={handleSOS}
            ...
            >
                <AlertTriangle className="w-5 h-5 animate-pulse"/> SOS
            </button>
            
            {pendingSync > 0 && (
                <button 
                    onClick={handleManualSync}
                    className="flex-1 bg-orange-500 text-white h-14 rounded-2xl font-black uppercase text-[10px] flex items-center justify-center gap-2 shadow-lg animate-pulse"
                >
        ...
        {view === 'dashboard' && (
            <div className="space-y-6 pb-24">
                    ...
                    <div className="flex justify-between items-start mb-6 relative z-10">
                        <div>
                            ...
                            <p className={`${isHighContrast ? 'text-black' : 'text-blue-400'} text-[10px] font-black uppercase tracking-widest flex items-center gap-2`}>
                                <Brain className="w-4 h-4"/> Routage Souverain
                            </p>
                        </div>
                    ...
                    <div className="space-y-3 relative z-10">
                        {patrolRecs.length === 0 ? (
                            <button onClick={generatePatrol} className={`w-full py-4 rounded-2xl font-black uppercase text-xs border-2 ${isHighContrast ? 'border-black' : 'border-blue-500/30 text-blue-400 bg-blue-600/10'}`}>
                                Analyser terrain pour patrouille
                            </button>
                        ) : patrolRecs.map(rec => {
                ...
                <button 
                    ...
                    className={`w-full h-44 flex flex-col items-center justify-center rounded-[3.5rem] shadow-2xl transition-all active:scale-95 border-b-[12px] ${
                        isHighContrast ? 'bg-black text-white border-slate-700' : 'bg-blue-600 text-white border-blue-800'
                    }`}
                >
                    <QrCode className="w-20 h-20 mb-3"/>
                    <span className="font-black uppercase tracking-[0.3em] text-xl leading-none">Scanner Badge</span>
                    <span className="text-[9px] font-bold uppercase mt-2 opacity-60 tracking-widest">Identité Numérique de l'Étal</span>
                </button>

                {/* STATS */}
                <div className="grid grid-cols-2 gap-4">
                    <Card className={`p-8 rounded-[3rem] flex flex-col justify-between ${isHighContrast ? 'border-8 border-black' : 'border-none shadow-xl bg-white'}`}>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Caisse Cash</p>
                        <h4 className="text-3xl font-black text-green-600 tracking-tighter leading-none mt-2">{formatCurrency(cashInHand)}</h4>
                    </Card>
                    <button 
                        onClick={() => { vibrate('warning'); setView('closing'); }}
                        className={`p-8 rounded-[3rem] flex flex-col justify-between text-left border-4 transition-all ${
                            isHighContrast ? 'border-black bg-white' : 'bg-white border-slate-100 shadow-xl'
                        }`}
                    >
                        <LogOut className="w-8 h-8 text-slate-400 mb-4"/>
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Sortie de Shift</span>
                    </button>
                </div>
            </div>
        )}
        ...
        {view === 'scanner' && (
            <div className="fixed inset-0 z-[100] bg-white p-4 flex flex-col animate-slide-up">
                <div className="flex justify-between items-center mb-6">
                    <button onClick={() => setView('dashboard')} className="p-4 bg-slate-100 rounded-2xl transition-transform active:scale-90"><X/></button>
                    <h3 className="font-black uppercase tracking-tighter text-xl">Pointage Terrain</h3>
        ...
        {view === 'closing' && (
            <div className="fixed inset-0 z-[100] bg-white p-6 flex flex-col animate-fade-in overflow-y-auto">
                <div className="flex justify-between items-center mb-10">
                    ...
                    <h3 className="font-black uppercase tracking-tighter text-xl">Dépôt Trésorerie</h3>
                    <div className="w-12"></div>
                </div>

                <div className="flex-1 space-y-8">
                    <div className="text-center">
                        <p className="text-slate-400 font-black uppercase text-xs tracking-[0.2em] mb-2">Cash à Verser</p>
                        <h4 className="text-7xl font-black text-slate-900 tracking-tighter leading-none">{formatCurrency(cashInHand)}</h4>
                    </div>

                    <div className="bg-slate-50 p-8 rounded-[3.5rem] space-y-6 border-2 border-slate-100">
                        <div className="flex justify-between items-center font-bold">
                            <span className="text-slate-500 uppercase text-[10px] tracking-widest">Quittances émises</span>
                            <span className="text-slate-900 font-black text-xl">{transactions.length}</span>
                        </div>
                        <div className="flex justify-between items-center font-bold">
                            <span className="text-slate-500 uppercase text-[10px] tracking-widest">Amendes notifiées</span>
                            <span className="text-slate-900 font-black text-xl">{sanctions.filter(s => s.issuedBy === currentUser.id).length}</span>
                        </div>
                        <div className="pt-6 border-t border-slate-200">
                            <p className="text-[9px] text-slate-400 uppercase font-black mb-4 tracking-widest">Protocoles de Fin de Shift</p>
                            <div className="space-y-3">
                                <div className="flex items-center gap-3 p-5 bg-white rounded-3xl border border-slate-200">
                                    <div className="w-6 h-6 rounded-full bg-green-500 flex items-center justify-center text-white"><CheckCircle2 className="w-4 h-4"/></div>
                                    <span className="font-bold text-xs">Positions GPS archivées</span>
                                </div>
                                <div className="flex items-center gap-3 p-5 bg-white rounded-3xl border border-slate-200">
                                    <div className="w-6 h-6 rounded-full bg-blue-500 flex items-center justify-center text-white"><ShieldCheck className="w-4 h-4"/></div>
                                    <span className="font-bold text-xs">Intégrité des reçus certifiée</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4 pt-4">
                        <Button 
                            className="w-full h-24 bg-slate-900 text-white rounded-[2.5rem] font-black uppercase text-lg tracking-widest shadow-2xl active:scale-95"
                        ...
                        <button 
                            ...
                            className="w-full py-4 font-black text-slate-400 uppercase text-[10px] tracking-widest hover:text-slate-900 transition-colors"
                        >
                            RETOUR AU TERRAIN
                        </button>
                    </div>
